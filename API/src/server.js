...
app.post("/api/docs/login", async (req, res) => {
  ...
  try {
    const [users] = await pool.execute("SELECT id, username, password, role FROM users WHERE username = ?", [username]);
...
server.listen(PORT, async () => {
  ...
  if (process.env.NGROK_AUTHTOKEN) {
    try {
      ...
      // Auto-write ngrok URL to app/.env so Flutter picks it up
      const fs = require("fs");
      const timeout = process.env.API_TIMEOUT || 30000;
      fs.writeFileSync("./app/.env", `API_BASE_URL=${ngrokUrl}\nAPI_TIMEOUT=${timeout}`);
      console.log("✅ app/.env updated with ngrok URL");
    } catch (err) {
      console.warn("⚠️  ngrok failed to start:", err.message);
    }
  } else {
    console.warn("⚠️  NGROK_AUTHTOKEN not set — skipping ngrok tunnel");
    // Write emulator localhost alias so Flutter still works
    const fs = require("fs");
    const timeout = process.env.API_TIMEOUT || 30000;
    fs.writeFileSync("./app/.env", `API_BASE_URL=http://10.0.2.2:${PORT}\nAPI_TIMEOUT=${timeout}`);
    console.log(`✅ app/.env updated with emulator URL: http://10.0.2.2:${PORT}`);
  }
});
