...
router.post("/", requireRole(["admin", "superadmin"]), uploadCategory.single("category_image"), async (req, res) => {
  try {
    ...
    const [result] = await pool.execute("INSERT INTO categories (name, description, category_image) VALUES (?, ?, ?)", [
      name,
      description ?? null,
      categoryImage,
    ]);
...
router.put("/:id", requireRole(["admin", "superadmin"]), uploadCategory.single("category_image"), async (req, res) => {
  try {
    ...
    const [oldCategories] = await pool.execute("SELECT category_image FROM categories WHERE id = ?", [req.params.id]);
    ...
    const [result] = await pool.execute(
      ...
      [name, description ?? null, categoryImage, req.params.id],
    );
...
router.delete("/:id", requireRole(["superadmin"]), async (req, res) => {
  try {
    ...
    const [products] = await pool.execute("SELECT COUNT(*) as count FROM products WHERE category_id = ?", [req.params.id]);
    ...
    const [categories] = await pool.execute("SELECT category_image FROM categories WHERE id = ?", [req.params.id]);
